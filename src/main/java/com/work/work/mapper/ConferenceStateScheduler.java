...
public class ConferenceStateScheduler {
    ...
    public void checkAndUpdateConferenceStates() {
        ...
        List<Conference> approvedConferences = conferenceMapper.selectConferencesByState("APPROVED");
        for (Conference conference : approvedConferences) {
            ...
            if (now.isAfter(conference.getStartTime())) {
                conferenceMapper.updateState(conference.getId(), "ONGOING");
            }
        }

        // 2. 检查需要从ONGOING转为COMPLETED的会议
        List<Conference> ongoingConferences = conferenceMapper.selectConferencesByState("ONGOING");
        for (Conference conference : ongoingConferences) {
            if (now.isAfter(conference.getEndTime())) {
                conferenceMapper.updateState(conference.getId(), "COMPLETED");
            }
        }
    }
}
